...
class Command(BaseCommand):
    ...
    def handle(self,*args,**kwargs):
        ...
        for n,row in enumerate(data,1):
            sec,_=Section.objects.get_or_create(code=row['section_code'], defaults={'title':row['section_title'],'order':int(row['section_code']) if row['section_code'].isdigit() else n})
            sub,_=SubSection.objects.get_or_create(code=row['subsection_code'], defaults={'section':sec,'title':row['subsection_title'],'order':n})
